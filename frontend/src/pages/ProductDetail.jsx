...
const ProductDetail = () => {
    ...
    const fetchData = async () => {
      try {
        const [pRes, rRes] = await Promise.all([
          ...
          axios.get(`/api/reviews/seller/${id}`).catch(() => ({ data: [] })),
        ]);
  ...
  const handleOrder = async () => {
    ...
    setOrdering(true);
    try {
      await axios.post('/api/orders', { product_id: id });
      setOrderMsg('success');
    } catch (err) {
  ...
  const imageUrl = product.image_url ? `http://localhost:5000${product.image_url}` : 'https://via.placeholder.com/600x400?text=No+Image';
    ...
    <div className="page">
      <div className="container">
        <div style={styles.grid}>
          ...
          <div style={styles.info}>
            ...
            <div style={styles.ratingRow}>
              ...
              <span style={styles.ratingText}>{product.avg_rating} ({product.review_count} reviews)</span>
            </div>
            ...
            {orderMsg === 'success' ? (
              <div className="alert alert-success">✅ Order placed! Check <a href="/orders" style={{ color: 'var(--success)' }}>My Orders</a> for status.</div>
            ) : orderMsg ? (
        ...
        <div style={{ marginTop: 48 }}>
          <h2 style={{ fontFamily: 'Syne, sans-serif', fontSize: 22, marginBottom: 20 }}>Reviews</h2>
          {reviews.length === 0 ? (
              ...
              {reviews.map(r => (
                <div key={r.id} className="card" style={styles.reviewCard}>
                  <div style={styles.reviewHeader}>
                    ...
                    <span style={{ color: '#7A6A58', fontSize: 12 }}>{new Date(r.created_at).toLocaleDateString()}</span>
                  </div>
                  {r.comment && <p style={{ color: '#2D2015', fontSize: 14, marginTop: 8 }}>{r.comment}</p>}
                </div>
              ))}
            </div>
          )}
