...
const ReviewModal = ({ order, onClose, onDone }) => {
  ...
  const submit = async () => {
    setLoading(true);
    try {
      await axios.post('/api/reviews', { order_id: order.id, rating, comment });
      onDone();
    } catch (err) {
...
const MyOrders = () => {
  ...
  useEffect(() => {
    ...
    axios.get('/api/orders/mine').then(res => setOrders(res.data)).finally(() => setLoading(false));
  }, [user]);
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
            {orders.map(o => (
              <div key={o.id} className="card" style={styles.orderCard}>
                <img src={o.image_url ? `http://localhost:5000${o.image_url}` : 'https://via.placeholder.com/64'} alt="" style={styles.thumb} />
